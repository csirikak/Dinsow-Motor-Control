...
int serialL; // global variable for steering. -1000 to 1000
int serialR; // global variable for speed. -1000 to 1000
int speed;
...
int main(void) {
  ...
  while(1) {
    ...
    #ifdef CONTROL_SERIAL_USART2
      cmd1 = CLAMP((int)command.serialL, -1000, 1000);
      cmd2 = CLAMP((int)command.serialR, -1000, 1000);
    ...
    // ####### LOW-PASS FILTER #######
    serialL = serialL * (1.0 - FILTER) + cmd1 * FILTER;
    serialR = serialR * (1.0 - FILTER) + cmd2 * FILTER;


    // ####### MIXER #######
    speedR = CLAMP(serialR * SPEED_COEFFICIENT, -1000, 1000);
    speedL = CLAMP(serialL * SPEED_COEFFICIENT, -1000, 1000);
    speed = (speedR + speedL) / 2.0;
